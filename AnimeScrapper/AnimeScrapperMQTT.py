...
# files paths
statusFile = "config/anime-status.json"
dbConfigFile = "config/scrapper-conf-V5-anime.json"
...
# function to update mqtt error status
def mqttError(message):
    try:
        publish.single(statusTopic, str(datetime.now())+">>"+str(message), 2, True, brokerUrl + "/error")
        mqttUpdate(message)
    except: print("Couldn't update mqtt status!")
    return
...
if not finished:
    logging.info(f"Scrapping anime data from Id: {lastId} to Id: {maxId}")
            ...
            while True:
                ...
                if animeData:
                    ...
                    if insertStatus:
                        ...
                        mqttError(f"Error while inserting Id: {x} data into database!")
                    break
